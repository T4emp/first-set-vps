...
iptables_rules() {
    ...
    #OTHER PORTS
    iptables -A INPUT -p tcp --dport 7443 -m state --state NEW -m hashlimit \
        --hashlimit-name conn_443 \
        --hashlimit-above 200/min \
        --hashlimit-burst 300 \
        --hashlimit-mode srcip \
        --hashlimit-htable-expire 60000 \
        -j DROP
    iptables -A INPUT -p tcp --dport 7443 -m state --state NEW -j ACCEPT
    iptables -A INPUT -p tcp --dport 8443 -m state --state NEW -m hashlimit \
        --hashlimit-name conn_443 \
        --hashlimit-above 200/min \
        --hashlimit-burst 300 \
        --hashlimit-mode srcip \
    ...
    iptables -A INPUT -p tcp --dport 9443 -m state --state NEW -m hashlimit \
        --hashlimit-name conn_443 \
        --hashlimit-above 200/min \
        --hashlimit-burst 300 \
        --hashlimit-mode srcip \
        --hashlimit-htable-expire 60000 \
        -j DROP
    iptables -A INPUT -p tcp --dport 9443 -m state --state NEW -j ACCEPT
    iptables -A INPUT -p tcp --dport 10443 -m state --state NEW -m hashlimit \
        --hashlimit-name conn_443 \
        --hashlimit-above 200/min \
        --hashlimit-burst 300 \
        --hashlimit-mode srcip \
        --hashlimit-htable-expire 60000 \
        -j DROP
    iptables -A INPUT -p tcp --dport 10443 -m state --state NEW -j ACCEPT
	  #PORT SCAN
